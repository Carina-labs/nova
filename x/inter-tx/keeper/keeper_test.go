...
func (suite *KeeperTestSuite) setZone(num int) []intertxtypes.RegisteredZone {

	addr := make([]sdk.AccAddress, 0)
	zones := make([]intertxtypes.RegisteredZone, 0)

	for i := 0; i < num; i++ {
		addr = append(addr, suite.GenRandomAddress())
		zones = append(zones, intertxtypes.RegisteredZone{
