...
func TestGenesis_ValidateState(t *testing.T) {
	...
	validState := func() *types.AirdropState {
		return &types.AirdropState{
	...
	gs := types.NewGenesisState([]*types.AirdropState{validState()}, validAirdrop())
	require.NoError(t, gs.Validate())

	// check recipient is invalid address
	state := validState()
	state.Recipient = invalidAddr
	gs = types.NewGenesisState([]*types.AirdropState{state}, validAirdrop())
	require.Error(t, gs.Validate())
